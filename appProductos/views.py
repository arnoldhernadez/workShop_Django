...
def verProductosCategoria(request, idCategoria): #Consultar categorias
    idCat = int(idCategoria)
    nombreCat = Categoria.objects.get
...
def verProducto(request, idProd, msj = None): #Consultar
    idProd = int(idProd)
    regProducto = Producto.objects.get(id= idProd) #ensamblar context
    context = {
    ...
    if msj:
        context['mensaje']= msj #renderizar
...
def eliminarCarrito(request, id): #Consultar el reg y cambiar estado
    regCarrito = Carro.objects.get(id=id) 
    regCarrito.estado = 'anulado' #guardar en BD
    regCarrito.save() #Desplegar el carrito 
...
def consultarCarro(request): 
    ...
    for prod in listaCarrito: 
        ...
        context = { 'titulo': 'Productos en el carrito de compras', 
                   'carrito': listado, 
                   'subtotal': subtotal, 
                   'iva': int(subtotal) * 0.19, 'envio': envio, 
